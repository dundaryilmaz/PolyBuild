...
contains
  ...
  subroutine  in_chain_neighbors(chain,mer,atom,mers,atoms)
    ...
    function x_linker(chaini,chainj,meri,merj,ic,jc,delta) result(iostat)
        ...
        !shifti=shifti/vecmag(shifti)*(vecmag(shifti)-0.3)
        !shiftj=-shifti
        !iostat=shift_mers(chaini,meri-2,shifti)
        !iostat=shift_mers(chainj,merj-2,shiftj)
        !iostat=shift_mers(chaini,meri+2,shifti)
        !iostat=shift_mers(chainj,merj+2,shiftj)
        !print*, 'Second Shift : ', vecmag(shifti)
!
        !shifti=shifti/vecmag(shifti)*(vecmag(shifti)-0.3)
        !shiftj=-shifti
        !iostat=shift_mers(chaini,meri-3,shifti)
        !iostat=shift_mers(chainj,merj-3,shiftj)
        !iostat=shift_mers(chaini,meri+3,shifti)
        !iostat=shift_mers(chainj,merj+3,shiftj)
        print*, 'Initial Distance Between Carbon Atoms: ', initial_dist
        !print*, '3rd : ', vecmag(shifti)

        !shifti=shifti/vecmag(shifti)*(vecmag(shifti)-0.3)
        !shiftj=-shifti
        !iostat=shift_mers(chaini,meri-4,shifti)
        !iostat=shift_mers(chainj,merj-4,shiftj)
        !iostat=shift_mers(chaini,meri+4,shifti)
        !iostat=shift_mers(chainj,merj+4,shiftj)
        !print*, 'Initial Distance Between Carbon Atoms: ', initial_dist
        !print*, '4th : ', vecmag(shifti)
