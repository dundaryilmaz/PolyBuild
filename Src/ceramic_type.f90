...
contains
    function read_unit_cell(filename) result(new_ceramic)
        ...
        else if (file_format == 'Charge')then

        do i=1,n_atoms
            read(13,*) new_ceramic%atype(i),new_ceramic%charge(i),vec,&
                &      new_ceramic%chain(i) , new_ceramic%symbol(i),new_ceramic%mass(i)
            new_ceramic%coord(i,1) =vec(1)*new_ceramic%h_matrix(1,1)
            new_ceramic%coord(i,2) =vec(2)*new_ceramic%h_matrix(2,2)
            new_ceramic%coord(i,3) =vec(3)*new_ceramic%h_matrix(3,3)
        ...
        do i=1,nx
            do j=1,ny
             ...
             !           if( (i .eq. 1) .and. (base_ceramic%chain(q) .eq. 1) ) cycle
                        ...
                        new_block%charge(cnt) =base_ceramic%charge(q)
...
101            call random_number(rn)
   ...
   function create_binary_defects(this,def_symbA,def_symbB,n_defs,def_treshold,direction) result(iostat)
       class(ceramic) :: this
       character(len=2) :: def_symbA,def_symbB
       integer :: n_defs
       integer :: iostat
       real*8  :: def_treshold,distance
       ...
       iostat=0
       n_atoms=this%n_atoms
       print*, 'Creating bin defects',n_defs
       i=0
       do while( i < n_defs )
...
101        del_atomA=int(n_atoms*rn)+1 ! pick random
           ...
           do j=1,this%n_atoms
               del_atomB=j
               if (del_atomB .eq. del_atomA) cycle
               if (this%deleted(del_atomB) .eq. 1) cycle
               if (this%symbol(del_atomB) .ne. def_symbB) cycle
               distance=pbc_dist(this%coord(del_atomA,:),this%coord(del_atomB,:))
               vecij= this%coord(del_atomB,:)-this%coord(del_atomA,:)
               if( distance > 3.0 .or. vecij(3) < 1.5) cycle
               this%deleted(del_atomB) = 1
               exit

               
               !print*,i,del_atomB,this%symbol(del_atomB),distance
           end do
           i=i+1
       end do
!       stop
