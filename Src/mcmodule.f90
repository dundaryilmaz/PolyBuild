 module mcmodule
   ...
   function self_chain_interaction(chainA) result(penalty)
     real*8 :: penalty
	 type(chain) :: chainA
	 integer :: i , j , k,mer_id
	 real*8 :: distance,mer_length
	 penalty =0.d0
     mer_id=chainA%mer_id
	 mer_length=base_mers(mer_id)%mer_length
...
!& 		  'Distance beads : ',i-2,i,distance,chainA%coord(i-2,:),chainA%coord(i,:)
		 if( distance < mer_length*1.7d0 ) then
		   penalty=penalty+exp(-1.5*(distance-base_mers(mer_id)%mer_length*1.7)/(base_mers(mer_id)%mer_length*1.7))
		 end if
	  end do
!	  pause
   end function
   ...
   function two_chain_interaction(chainA,chainB) result(penalty)
     ...
     integer :: i , j , k,mer_idA,mer_idB
     real*8 :: distance
     penalty=0.d0
     mer_idA=chainA%mer_id
     mer_idB=chainB%mer_id
!	 print*,mer_width,'---'
     do i=1,chainA%n_beads
       do j=1,chainB%n_beads
         distance=pbc_dist(chainA%coord(i,:),chainB%coord(j,:))
         if( distance > base_mers(mer_idA)%mer_width ) then !avg_bead_distance) then
           penalty=penalty+exp(-1.5*(distance/base_mers(mer_idA)%mer_width))
...
!&   		  'i : ',i,' j  ',j,distance,exp(-2.0*(distance/mer_width)),penalty
		   penalty=penalty+exp(1.5*(base_mers(mer_idA)%mer_width/distance))
         end if
       end do
     end do
!  print*,penalty
...
!	   write(*,'(A,x,2f8.2)') 'Distance', length,sqrt(dot_product(vec,vec))
       scale_cf=base_mers(mer_id)%mer_length/length
	   !write(*,'(i4,3f8.3)'),i,length,base_mer%mer_length,scale_cf
       vecA=vec*base_mers(mer_id)%mer_length
