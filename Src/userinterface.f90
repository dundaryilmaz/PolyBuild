...
integer :: iostat,ilo,iho,indx,n_words,ihi,i,cnt
integer :: n_mer_atoms,j,n_mcsteps,q,qaccp,p,qq,k,nfunctional
integer :: n_def_groups,n_cross_links,n_basis_mers,n_bindef_groups
...
100  read(11,'(A80)',end=101) cline
  if( cline(1:1) .eq. '#') goto 100
   ...
   if( word .eq. 'monomer') then
     call word_next_read(cline,word,done)
     read(word,*) n_basis_mers
     allocate(base_mers(n_basis_mers))
     print*,'number of basis mers : ',n_basis_mers
     do i=1,n_basis_mers
         read(11,'(A80)')cline
         print*,cline
         done=.true.
         ...
         if( .not. done) then
            call word_next_read(cline,word,done)
            if (word .eq. 'R')base_mers(i)%radical=.TRUE.
         end if
        
         print*, 'Monomer File : ',monomer_file
         iostat=base_mers(i)%create(monomer_file)
         write(*,'(A,x,A)') 'Monomer name :' , base_mers(i)%monomer_name
         write(12,'(A,x,A)') 'Monomer name :' , base_mers(i)%monomer_name
         write(*,'(A,x,i2)') 'Number of atoms in monomer : ',base_mers(i)%n_mer_atoms
         write(12,'(A,x,i2)') 'Number of atoms in monomer : ',base_mers(i)%n_mer_atoms
         write(12,'(A,x,3f8.4)')' Base Vector - 1 :',base_mers(i)%base_vecs(1,:)
         write(12,'(A,x,3f8.4)')' Base Vector - 2 :',base_mers(i)%base_vecs(2,:)
         write(12,'(A,3f8.4)')'Head ', base_mers(i)%headh
         write(12,'(A,3f8.4)')'Tail ', base_mers(i)%tailh
         write(*,'(A,f8.4)') 'Mer Width is :',base_mers(i)%mer_width
     end do
   end if
...
&                  def_treshold(i)
   ...
   if ( word .eq. 'binary_defects') then
       call word_next_read(cline,word,done)
       read(word,*) n_bindef_groups
       allocate(n_binary_defects(n_bindef_groups),def_symbolsA(n_bindef_groups), &
&      def_symbolsB(n_bindef_groups),def_block(n_bindef_groups),def_treshold(n_bindef_groups))
       do i=1,n_bindef_groups
           read(11,*) def_block(i),def_symbolsA(i),def_symbolsB(i),n_binary_defects(i),&
&                    def_treshold(i)
       end do            
   ...
   if ( word .eq. 'amorph_chain') then
     ...
     allocate( chainlist(n_amorph_chain_groups,3))
     do i=1, n_amorph_chain_groups
         read(11,*) chainlist(i,:)  ! 1 : base type 2: n chains 3 n_mers
     end do
   end if
   if ( word .eq. 'opls') then
       call word_next_read(cline,word,done)
       if ( word .eq. 'yes') opls_flag = .true.
       print*, 'Connection Table will be printed'
   end if
   ...
   if( word .eq. 'cross_links') then
     call word_next_read(cline,word,done)
     read(word,*) n_xlink_chains
     call word_next_read(cline,word,done)
     read(word,*) mers_per_chainx
     call word_next_read(cline,word,done)
     read(word,*)nxmers
...
!-----------------------------------------
n_chains=n_cont_chains+nfunctional+n_xlink_chains
n_amorph_chains=0
do i=1,n_amorph_chain_groups
    n_amorph_chains=n_amorph_chains+chainlist(i,2)
end do
n_chains=n_chains+n_amorph_chains
...
if(n_cont_chains .ne. 0 ) then
  do i=1, n_cont_chains
    chains(i)=build_continuous_chain(mers_per_chain_cont,i,'z',1)
  end do
...
if( n_amorph_chains .gt. 0  ) then
  ...
  cnt=0
  do i=1,n_amorph_chain_groups
      do j=1,chainlist(i,2)
          cnt=cnt+1
          chains(n_cont_chains+cnt)=create_chain(chainlist(i,3),n_cont_chains+cnt,chainlist(i,1))
       end do
  end do
...
!------------------------------------------
! Create Xlink Amorphous Chains
!------------------------------------------
if( n_xlink_chains .gt. 0  ) then
  do i=n_cont_chains+n_amorph_chains+1,n_cont_chains+n_amorph_chains+n_xlink_chains
    chains(i)=create_chain(mers_per_chainx,i,nxmers)
    total_penalty=self_chain_interaction(chains(i))
  end do
  write(*,"(i3,x,'Xlink Amorphous Chains Created')") n_xlink_chains  
  write(*,"(i3,x,'N mers' ,i3,x,'N xmers')") mers_per_chainx,nxmers
end if	
!------------------------------------------
...
if( n_cross_links > 0) then
call cross_linker(n_cross_links,cross_link_min_dist)
end if
!-----------------------------------------------------------
...
if( n_boxes > 0  ) then
  ...
  if (allocated(n_binary_defects)) then
      print*, 'Creating binary defects '
      do i=1,n_bindef_groups
          print*,def_symbolsA(i),def_symbolsB(i),n_binary_defects(i),def_treshold(i)
           iostat=blocks(def_block(i))%create_binary_defects(def_symbolsA(i),def_symbolsB(i),&
&                 n_binary_defects(i),def_treshold(i))           
      end do
  end if
...
if( opls_flag) iostat=connection_table(outputfilename)
end function inputscript_reader
subroutine cross_linker(nlinks,min_cross_dist)
  integer :: i,j,nlinks,nchains
  real*8  :: rn,dist,distmin,magij,delta,shift,min_cross_dist
  integer :: chaini,chainj,nlink,ntry
  real*8, dimension(3) :: veci,vecj,vecij,unitvecij
  integer :: meri, merj,ic,jc,ih,jh,iostat
  integer , dimension (:) , allocatable :: linked
    ...
    do while ( nlink < nlinks)
      ntry=ntry+1
      if( ntry > nlinks*1000*nchains) return
      !if ( mod(ntry,100) .eq. 0 ) print*,ntry, nlinks*100*nchains, nlink,nlinks
      call random_number(rn)
      ...
      if ( meri .lt. 5 .or. meri .gt. chains(chaini)%n_mers-5 ) cycle
      merj=int(chains(chainj)%n_mers*rn)+1
      if ( merj .lt. 5 .or. merj .gt. chains(chainj)%n_mers-5 ) cycle
      dist=pbc_dist(chains(chaini)%coord(meri,:),chains(chainj)%coord(merj,:))
      if ( dist > min_cross_dist ) cycle
      linked(chaini) = 1
      linked(chainj) = 1
      ic=1
      jc=1
      ...
      delta=magij
      print*, 'Delta: ',delta
      shift=(delta-1.75d0)/6
      print*,'Shift : ',unitvecij*shift,shift
      
      !iostat=chains(chainj)%translate_chain(+unitvecij*2.*shift)
      !iostat=chains(chaini)%translate_chain(-unitvecij*2.*shift)
      !veci=chains(chaini)%mers(meri)%coord(ic,:)
      !vecj=chains(chainj)%mers(merj)%coord(jc,:)
      !chains(chainj)%mers(merj)%coord(jc,:)=vecj+(unitvecij*shift)
      !chains(chaini)%mers(meri)%coord(ic,:)=veci-(unitvecij*shift)
      iostat=x_linker(chaini,chainj,meri,merj,ic,jc,1.65d0)
      vecj=chains(chainj)%mers(merj)%coord(jc,:)
      veci=chains(chaini)%mers(meri)%coord(ic,:)
      print*,'After shift : ', pbc_dist(veci,vecj)
      ...
      nlink=nlink+1
      print*, 'Cross Linked Chains : ',nlink,chaini,chainj
    end do
end subroutine
